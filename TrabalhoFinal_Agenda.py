...
Original file is located at
    https://colab.research.google.com/drive/1-0r1Fafl4pw2EByqYBIchX6YDvDI6xBd
"""
...
def listar(eventos):
  if len(eventos) == 0:
      print('\n\t  A agenda está vazia!')
  eventos_ord = sorted(eventos, key=lambda x: (x.data, x.hora))
  for i in range(len(eventos_ord)):
    if eventos_ord[i].data == None:
      del eventos_ord[i]
    else:
      print(
          '\nData: ', eventos_ord[i].data,
          '\nHora: ', eventos_ord[i].hora, 
          '\nDuração: ', eventos_ord[i].duracao, 
          '\nDescrição: ',eventos_ord[i].descricao 
          )
      print('#'*70)
